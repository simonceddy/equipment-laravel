...
function Dropdown({ children }) {
  ...
  const val = useMemo(() => ({ open, setOpen, toggleOpen }), [open]);
...
function DropdownLink({ className = '', children, ...props }) {
  return (
    <Link
      {...props}
      className={
                `block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-900 dark:focus:bg-gray-900 transition duration-150 ease-in-out ${
                  className}`
            }
    >
      {children}
